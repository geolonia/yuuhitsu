...
- [ ] T005 [P] Create project directory structure per plan.md: `src/cli/`, `src/cli/commands/`, `src/provider/`, `src/tasks/`, `src/templates/`, `tests/unit/`, `tests/integration/`
...
- [ ] T010b Implement Gemini adapter in `src/provider/gemini.ts` — use @google/genai, read GOOGLE_API_KEY from env, implement AIProvider interface
- [ ] T010c Implement Ollama adapter in `src/provider/ollama.ts` — use openai SDK with localhost base URL, no auth, implement AIProvider interface
- [ ] T011 Implement provider factory in `src/provider/index.ts` — return appropriate adapter instance based on provider name (claude/gemini/ollama)
- [ ] T012 Unit test for provider factory in `tests/unit/provider/provider.test.ts` — claude config, gemini config, ollama config, unsupported provider error, missing API key error
...
- [ ] T024 [US1] Add error handling: invalid/missing API key (ANTHROPIC_API_KEY/GOOGLE_API_KEY) → clear message without exposing key, file not found, empty file, network error → suggest Ollama
...
**Goal**: Seamless switching between Claude, Gemini, and Ollama by editing config

**Independent Test**: Change config from `provider: claude` to `provider: gemini` or `provider: ollama`, run translate command, verify it works with each provider

### Tests for User Story 3

- [ ] T030 [P] [US3] Unit test for provider switching in `tests/unit/provider/switching.test.ts` — claude→gemini→ollama config change, model format validation, error on unsupported provider
...
- [ ] T042 [US5] Add capability check — detect if provider/model supports web search, suggest cloud provider (Claude/Gemini) if not
...
- [ ] T051 [P] Implement exponential backoff retry (FR-006) — configure retry logic in each adapter (max 3 retries, 1s start)
...
| Phase 2: Foundational | T008-T017 (13, incl. T010a-c) | — |
